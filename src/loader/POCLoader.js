...
function loadPCO(fMnoString, url, binaryLoader, callback) {
  const pco = new PointCloudOctreeGeometry();
  pco.url = url;
  let fMno = JSON.parse(fMnoString);

  let version = new Version(fMno.version);

  pco.octreeLoader = binaryLoader;
...
function HttpBinaryLoad(url, successCallback, failCallback) {
...
export class POCLoader {
		...
		try {
			...
			xhr.onreadystatechange = function () {
			  if (xhr.readyState === 4 && (xhr.status === 200 || xhr.status === 0)) {
                            loadPCO(xhr.responseText, url, HttpBinaryLoad, callback)
			  }
			};
  ...
  static loadS3(s3, bucket, name, callback) {
    const s3BinaryLoad = (path, successCallback, failCallback) => {
      s3.getObject({Bucket: bucket,
                   ...
                   (err, data) => {
                     if (err) {
                       ...
                       successCallback(data.Body.buffer);
                     }});
    };
    ...
    s3.getObject({Bucket: bucket,
                 ...
                 (err, data) => {
                   if (err) {
                     ...
                     loadPCO(string, s3Directory, s3BinaryLoad, callback);
                   }});
  }
