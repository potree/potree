	...
	addPoints(data){
		...
		let createNewBatch = () => {
			...
			if(data.data.dualPlusConfidence) {
				let dualDistance = new Uint8Array(data.data.dualPlusConfidence.map(function(DPC) { return unpackDual(DPC).distFlag }));
				let dualReflectivity = new Uint8Array(data.data.dualPlusConfidence.map(function(DPC) { return unpackDual(DPC).intenFlag }));
				let confidence = new Uint8Array(data.data.dualPlusConfidence.map(function(DPC) { return unpackConfidence(DPC).confidence }));
